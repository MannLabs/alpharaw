...
def find(spectrum_df, peak_df, centroid_tol=8, max_gap=2,
    ...
    hill_ptrs = split_hills(hill_ptrs, hill_data, int_data, hill_split_level=hill_split_level, window = hill_smoothing) #hill lenght is inthere already
    logging.info(f'After split hill_ptrs {len(hill_ptrs):,}')

    hill_data, hill_ptrs = filter_hills(hill_data, hill_ptrs, int_data, hill_check_large = hill_check_large, window=hill_smoothing)
    ...
    feature_df, lookup_idx = feature_finder_report(query_data, isotope_patterns, isotope_charges, iso_idx, stats, sortindex_, hill_ptrs, hill_data)
    ...
    feature_df["last_isotope_mz"] = feature_isotopes_df.groupby("feature_id").tail(1).mz_average.values
