...
# local
import alpharaw
from alpharaw.utils import pjit
from alpharaw.utils.hdf import create_hdf_group_from_dict, create_dict_from_hdf_group
import alpharaw.tempmmap as tm
...
def init_bruker_dll(bruker_dll_file_name: str = BRUKER_DLL_FILE_NAME):
    ...
    bruker_dll_file_name : str
        ...
        Default is alpharaw.bruker.BRUKER_DLL_FILE_NAME.
    ...
    bruker_dll.tims_set_num_threads(pjit.MAX_THREADS)
...
def read_bruker_binary(
    ...
    frames : pd.DataFrame
        ...
        These can be acquired with e.g. alpharaw.bruker.read_bruker_sql.
    bruker_d_folder_name : str
    ...
    if compression_type == 1:
        process_frame_func = pjit.threadpool(
            process_frame,
            thread_count=1
        )
    else:
        process_frame_func = pjit.threadpool(process_frame)
    process_frame_func(
        range(1, len(frames)),
        tdf_bin_file_name,
        tims_offset_values,
...
class TimsTOF(object):
    ...
    def _import_data_from_d_folder(
        ...
        if (mobility_estimation_from_frame != 0) and calibration_available:
            import ctypes
            with alpharaw.bruker.open_bruker_d_folder(
                bruker_d_folder_name
...
def set_precursor(
    ...
    IMPORTANT NOTE: This function is decorated with alpharaw.utils.pjit.pjit.
...
def parse_keys(data: TimsTOF, keys) -> dict:
    ...
    keys : tuple
        ...
        See `alpharaw.bruker.convert_slice_key_to_int_array` and
        `alpharaw.bruker.convert_slice_key_to_float_array` for more details.
...
@pjit.pjit(thread_count=1, include_progress_callback=False)
...
def save_as_mgf(
    ...
    with open(full_file_name, "w") as infile:
        for index in pjit.progress_callback(
