...
# %% auto 0
__all__ = ['match_one_raw_with_numba', 'PepSpecMatch']
...
class PepSpecMatch:
    ...
    def _add_need_columns_to_psm_df(self,
        psm_df:pd.DataFrame,
        ms_reader:MSData_Base
    ):
        add_spec_info_list = []
    ...
    def get_ms2_peaks(self,
        ...
        **kwargs
        ...
        return ms_data.get_peaks(spec_idx)
    ...
    def _match_ms2_one_raw_numba(self, raw_name, df_group):
        if raw_name in self._ms_file_dict:
            if isinstance(self._ms_file_dict[raw_name], MSData_Base):
                ms_reader = self._ms_file_dict[raw_name]
            else:
                ms_reader = ms_reader_provider.get_reader(
                    self._ms_file_type
                )
                ms_reader.import_raw(self._ms_file_dict[raw_name])
