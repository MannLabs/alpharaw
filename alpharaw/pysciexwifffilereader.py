...
from alpharaw.utils.centroiding import centroid_data
...
class WillFileReader:
    ...
    def load_sample(self, sample_id:int, centroid:bool=True):
        ...
        for j in range(exp_list[0].Details.NumberOfScans):
            for i in range(self.msSample.ExperimentCount):
                ...
                if massSpectrum.NumDataPoints <= 0: continue
                details = exp.Details
                massSpectrumInfo = exp.GetMassSpectrumInfo(j)
                peak_mz_array_list.append(
                    DotNetArrayToNPArray(massSpectrum.GetActualXValues())
                )
                peak_intensity_array_list.append(
                    DotNetArrayToNPArray(massSpectrum.GetActualYValues())
                )
                ...
                # rt_list.append(exp.GetRTFromExperimentScanIndex(j))
                ...
                # BasePeakMz = (max_idx == -1) ? 0.0 : actualXValues[max_idx],
                # BasePeakIntensity = (float)((max_idx == -1) ? 0.0 : actualYValues[max_idx]),
                ...
                ms_level_list.append( massSpectrumInfo.MSLevel)
                ...
                if ms_level_list[-1] > 1:
                    ...
                    # scanInfo.ScanMode = WiffFile.ScanMode.Centroid
